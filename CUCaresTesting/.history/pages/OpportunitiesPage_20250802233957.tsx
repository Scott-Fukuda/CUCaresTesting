...
const OpportunitiesPage: React.FC<OpportunitiesPageProps> = ({
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    return opportunities
      .filter(opp => {
        if (causeFilter !== 'All' && opp.cause !== causeFilter) return false;

        const oppDate = parseDateAsLocal(opp.date);

        if (dateFilter) {
          const filterDate = parseDateAsLocal(dateFilter);
          if (
            oppDate.getFullYear() !== filterDate.getFullYear() ||
            oppDate.getMonth() !== filterDate.getMonth() ||
            oppDate.getDate() !== filterDate.getDate()
          ) return false;
        }
